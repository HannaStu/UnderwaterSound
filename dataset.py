...
class EcossDataset:
    ...
    def filter_overlapping(self, visualize_overlap = False) -> None:
        ...
        #preprocess the numerical data to enforce float format not string
        columns_to_convert = ["tmin", "tmax", "fmin", "fmax"]
    ...
    def _extract_overlapping_info(self):
        ...
        for _,row in self.df.iterrows():
            if pd.isna(row["overlapping"]): #bc of this not possible FALSE in datasets needs to be empty
    ...
    @staticmethod
    def _parse_overlapping_field(overlapping_str: str) -> list[tuple]: #does convert strings to float for the overlapping info col
        """
